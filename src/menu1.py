...
def menu1(root):
    ...
    # Название метода
    title_label = tk.Label(win, text="Вывод данных в определенный момент времени", font=title_font, bg='#333333', fg='white')
    ...
    # Момент времени
    time_label = tk.Label(param_frame, text="Момент времени (0 - 119)")
    time_label.pack()
    time_entry = tk.Entry(param_frame, width=20)
    time_entry.pack()
    ...
    button2 = ttk.Button(top_frame, text="Выполнить", style='My.TButton', 
                         command=lambda: run_method(file_entry.get(), int(radius_entry.get()), int(time_entry.get()), 
...
def run_method(dir_input, radius, time_index, lat, lon, dir_output):
    start_time = time.time()
    export_tvh_onetime.output_main(time_index, dir_input, radius, lat, lon, dir_output)
