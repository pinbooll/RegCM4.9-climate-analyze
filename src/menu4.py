...
def menu4(root):
    ...
    # Название метода
    title_label = tk.Label(win, text="Вывод распределения и анализ данных (несколько месяцев)", font=title_font, bg='#333333', fg='white')
    ...
    file_entry.insert(0, "Введите путь к папке с данными модели или выберите папку...")
    file_entry.bind("<FocusIn>", lambda args: file_entry.delete(0, 'end'))
    file_entry.pack(side='left', padx=5)
    file_button = ttk.Button(file_frame, text="Выбрать папку", style='My.TButton', command=lambda: select_folder(file_entry))
